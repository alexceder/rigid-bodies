...
To calculate the unknown impulse force $j$ we first only take into count the objects velocities. With the equations \ref{eq:physics:3} for both object, positive impulse part for object A and negative for B, and with \ref{eq:physics:2} we can derive the impuls \ref{eq:physics:4}.
\begin{equation}
...
The angular velocity is obtained as shown in \ref{eq:physics:5}, where the impulse part is positive for the object A and negative for B. $r_{AP}$ and $r_{BP}$ is the perpendicular radius from the center of mass to the collision point of the two objects.
...
To get the total impulse force the angular velocity also is taken into account and in the same approach as previous the result shown down below \ref{eq:physics:6}. In the equation M is the mass of the object, and moment of inertia \emph{I} is determined by the objects geometry.
...
Now that the impulse coefficient is known we can determine both equation \ref{eq:physics:3} och \ref{eq:physics:4}. The scalar is multiplied with the normalized collision vector in both equations. This is the basic simulation; when a collision occurs, solve for the properties of the collision and then apply the impulse to the objects velocity and angular velocity.
...
The normal of the collision will then be the vector from this closest position and the origin of the circle if by convention be have chosen the circle as body A and the circle as B. The closest collision point is illustrated in ~\ref{fig:closest}.
...
\begin{equation}
\Omega(t+1)=\Omega(t)+\omega(t+1)dt
\label{eq:euler:4}
\end{equation}
