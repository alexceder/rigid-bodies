...
CollisionPair()
:   _normal = vec2(0.0f, 0.0f);
    _collisions = vec2(0.0f, 0.0f);
    _penetration = 0.0f;
    // _A* = RigidBody();
    // _B* = RigidBody();
    { };

CollisionPair(RigidBody* A, RigidBody* B, vec2 normal, vec2 collisions, float penetration)
:   *_A = A, 
    *_B = B,
    _normal = normal;
...
float calculateImpulse(CollisionPair cp)    //cp bästa vi kom på
{   
    /**********************************************
...
void applyImpulse(float impulse) //var float innan. ska den verkligen returna något?
{
    /**********************************************
...
vec2 calculateCollisionNormal(RigidBody* A, RigidBody* B)
{
